...
"""
    ...
    if pname[0] == '"':
        pname = pname[1:-1]
    elif (len(pname) >= 1 and pname[0] == ".") or (
        len(pname) >= 2 and pname[1] == ":" and os.path.exists(pname)
    ):
        print("INFO:安装一个本地文件")
    ...
    if sys.argv[2] == "install":
        ...
        E_disk = tools.find_disk()
        print("INFO:当前启动盘 [" + E_disk + "]")
        shutil.copyfile(
            f"dist/dashedgeless_{D_version}_dashedgeless (bot).7z",
            config.PLUGIN_PATH + f"dashedgeless_{D_version}_dashedgeless (bot).7z",
        )
        print("INFO:安装完成")
    if sys.argv[2] == "build":
        print("INFO:编译程序")
